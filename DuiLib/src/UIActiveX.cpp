...
public:
    ...
    STDMETHOD(GetWindowContext)(IOleInPlaceFrame **ppFrame, IOleInPlaceUIWindow **ppDoc, LPRECT lprcPosRect,
...
STDMETHODIMP CActiveXCtrl::ParseDisplayName(IBindCtx *pbc, LPOLESTR pszDisplayName, ULONG *pchEaten,
                                            IMoniker **ppmkOut)
{
...
LRESULT CActiveXWnd::OnPrint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
    ...
    m_pOwner->m_pViewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, NULL, (HDC)wParam, (RECTL *) &rcClient, NULL,
                                  NULL, NULL);
...
void CActiveXUI::SetCLSID(const LPCTSTR pstrCLSID)
{
    CLSID clsid = { 0 };
    OLECHAR szCLSID[100] = { 0 };
#ifndef _UNICODE
    ::MultiByteToWideChar(CP_ACP, 0, pstrCLSID, -1, szCLSID, LENGTHOF(szCLSID) - 1);
#else
    ...
    if (clsid != IID_NULL) { m_clsid = clsid; }
}
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if (!m_sModuleName.IsEmpty())
    {
        ...
        if (hModule != NULL)
        {
            ...
            DllGetClassObjectFunc aDllGetClassObjectFunc = (DllGetClassObjectFunc)::GetProcAddress(hModule,
                                                           "DllGetClassObject");
