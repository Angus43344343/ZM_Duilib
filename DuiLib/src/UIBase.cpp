...
static const DUI_MSGMAP_ENTRY *DuiFindMessageEntry(const DUI_MSGMAP_ENTRY *lpEntry, TNotifyUI &msg)
{
	...
	//2021-09-09 zm 解决Menu的pSender为nullptr的特殊情况
	CDuiString sCtrlName = (nullptr != msg.pSender) ? msg.pSender->GetName() : nullptr;
...
HWND CWindowWnd::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, int x, int y,
                        int cx, int cy, HMENU hMenu)
{
    ...
    m_sTitle = pstrName;
    m_sTitleOrig = m_sTitle;
    CPaintManagerUI::ProcessMultiLanguageTokens(m_sTitle);
    m_hWnd = ::CreateWindowEx(dwExStyle, GetWindowClassName(), m_sTitle.GetData(),
