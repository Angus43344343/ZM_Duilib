...
#define _HSLToRGB(h,s,l) (0xFF << 24 | HSLToRGB((float)h / 360.0f,(float)s / 200.0f,l / 200.0f))
	...
	{
		return DUI_CTR_PALETTE;
	}

	LPVOID CColorPaletteUI::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_PALETTE) == 0) return static_cast<CColorPaletteUI*>(this);
	...
	{
		if (m_stThumbImage.sDrawString == pszImage && m_stThumbImage.pImageInfo != NULL) { return; }

		m_stThumbImage.Clear();
		m_stThumbImage.sDrawString = pszImage;
		NeedUpdate();
	}

	LPCTSTR CColorPaletteUI::GetThumbImage() const
	{
		return m_stThumbImage.sDrawString;
	}

	void CColorPaletteUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcscmp(pstrName, _T("paletteheight")) == 0) SetPalletHeight(ParseInt(pstrValue));
		else if (_tcscmp(pstrName, _T("barheight")) == 0) SetBarHeight(ParseInt(pstrValue));
		else if (_tcscmp(pstrName, _T("thumbimage")) == 0) SetThumbImage(ParseString(pstrValue));
	...
	void CColorPaletteUI::PaintPallet(HDC hDC)
	{
		...
		//拉伸模式将内存图画到控件上
		StretchBlt(hDC, m_rcItem.left, m_rcItem.top, m_rcItem.right - m_rcItem.left, m_nPalletHeight, m_MemDc, 0, 1, 360, 200, SRCCOPY);//调色板
		StretchBlt(hDC, m_rcItem.left, m_rcItem.bottom - m_nBarHeight, m_rcItem.right - m_rcItem.left, m_nBarHeight, m_MemDc, 0, 210, 200, m_nBarHeight, SRCCOPY);//单色变化区域

		RECT rcCurSorPaint = { m_ptLastPalletMouse.x - 4, m_ptLastPalletMouse.y - 4, m_ptLastPalletMouse.x + 4, m_ptLastPalletMouse.y + 4 };
		CRenderEngine::DrawImage(hDC, m_pManager, rcCurSorPaint, m_rcPaint, m_stThumbImage);//滑块图
