...
CDateTimeUI::CDateTimeUI()
{
    ...
    m_bReadOnly = false;
	m_bShowTime = false;//2021-09-07 zm
    m_pWindow = NULL;
...
void CDateTimeUI::DoEvent(TEventUI &event)
{
    ...
    {
        if (IsEnabled() && !IsReadOnly())//2021-09-07 zm 
        {
...
{
	//2021-09-07 zm 解决每次控件重绘时，控件会永久增宽26的bug
    SIZE szRect = CLabelUI::EstimateSize(szAvailable);
    szRect.cx += 26;
    return szRect;
	//m_cxyFixedLast += 26;
...
{
	//2021-09-07 zm 
	if (_tcscmp(pstrName, _T("showtime")) == 0) { SetShowTime(ParseBool(pstrValue)); }
	else if (_tcscmp(pstrName, _T("readonly")) == 0) { SetReadOnly(ParseBool(pstrValue)); }
