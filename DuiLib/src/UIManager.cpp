...
void CPaintManagerUI::Term()
{
	...
	CPaintManagerUI::RemoveSharedResAll();
...
void CPaintManagerUI::RemoveSharedResAll()
{
	//默认字体
	...
	for (int i = 0; i < m_SharedResInfo.m_CustomFonts.GetSize(); i++)
	{
		...
		{
			TFontInfo* pFontInfo = static_cast<TFontInfo*>(m_SharedResInfo.m_CustomFonts.Find(key, false));
			if (pFontInfo)
			{
	...
	for (int i = 0; i < m_SharedResInfo.m_ImageHash.GetSize(); i++)
	{
		...
		{
			TImageInfo* pImageInfo = static_cast<TImageInfo*>(m_SharedResInfo.m_ImageHash.Find(key, false));
			if (pImageInfo) CRenderEngine::FreeImage(pImageInfo);
		}
	}
	...
	for (int i = 0; i < m_SharedResInfo.m_AttrHash.GetSize(); i++)
	{
		...
		{
			CDuiString* pAttr = static_cast<CDuiString*>(m_SharedResInfo.m_AttrHash.Find(key, false));
	...
	for (int i = 0; i < m_SharedResInfo.m_StyleHash.GetSize(); i++)
	{
		...
		{
			CDuiString* pStyle = static_cast<CDuiString*>(m_SharedResInfo.m_StyleHash.Find(key, false));
	...
	for (int i = 0; i < m_SharedResInfo.m_MultiLanguageHash.GetSize(); i++)
	{
		...
		{
			CDuiString* pStyle = static_cast<CDuiString*>(m_SharedResInfo.m_MultiLanguageHash.Find(key, false));
			if (pStyle) delete pStyle;
		}
	}
...
const CDuiStringPtrMap& CPaintManagerUI::GetStyles(bool bShared) const
{
