...
void CScrollBarUI::DoEvent(TEventUI &event)
{
    ...
    if (event.Type == UIEVENT_TIMER && event.wParam == TIMERID_REPEAT)
    {
        ...
        if ((m_uThumbState & UISTATE_CAPTURED) != 0)
        {
            if (!m_bHorizontal)
            {
                if (m_pOwner != NULL)
                {
                    m_pOwner->SetScrollPos(CDuiSize(m_pOwner->GetScrollPos().cx,
            ...
            else
            {
                if (m_pOwner != NULL)
                {
                    m_pOwner->SetScrollPos(CDuiSize(m_nLastScrollPos + m_nLastScrollOffset,
    ...
    {
		//2021-09-08 zm 解决thumb处于hot状态后不再改变的bug
        if (!::PtInRect(&m_rcItem, event.ptMouse))
		//if (::PtInRect(&m_rcItem, event.ptMouse))
        {
            if (IsEnabled())
