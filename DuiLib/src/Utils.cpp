...
void CDuiString::Reserve(int nLen)
{
    if (nLen <= MAX_LOCAL_STRING_LEN) { return; }

    if (m_pstr == m_szBuffer)
    {
        m_pstr = static_cast<LPTSTR>(malloc((nLen + 1) * sizeof(TCHAR)));
        _tcscpy(m_pstr, m_szBuffer);
    }
    else
    {
        m_pstr = static_cast<LPTSTR>(realloc(m_pstr, (nLen + 1) * sizeof(TCHAR)));
    }
}
...
CDuiString &CDuiString::Append(LPCTSTR pstr)
{
...
LPTSTR CDuiString::GetData(int nLen)
{
    Reserve(nLen);
    return m_pstr;
}
...
const CDuiString &CDuiString::operator=(LPCWSTR lpwStr)
{
    if (lpwStr)
    {
        ...
        if (pstr != NULL) { ::WideCharToMultiByte(CP_ACP, 0, lpwStr, -1, pstr, cchStr, NULL, NULL); }
...
bool CDuiString::operator == (const CDuiString &str) const { return (Compare(str) == 0); }
bool CDuiString::operator != (const CDuiString &str) const { return (Compare(str) != 0); }
bool CDuiString::operator <= (const CDuiString &str) const { return (Compare(str) <= 0); }
bool CDuiString::operator < (const CDuiString &str) const { return (Compare(str) < 0);  }
bool CDuiString::operator >= (const CDuiString &str) const { return (Compare(str) >= 0); }
