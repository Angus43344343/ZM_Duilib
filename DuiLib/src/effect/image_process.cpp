...
BOOL CWarp::WarpImage(BYTE *pData, RECT &rcSrc, WarpRect &rcDes, RECT &rcBound, BYTE *imgDes,
                      Color color/* = Color::Transparent*/, BOOL bFill/* = TRUE*/)
{
    CWarpMatrix matrix;
...
BOOL CWarp::WarpImage(BYTE *pData, int srcDataWidth, int srcDataHeight, RECT &rcSrc, WarpRect &rcDes,
                      RECT &rcBound, BYTE *imgDes, Color color/* = Color::Transparent*/, BOOL bFill/* = TRUE*/)
{
    CWarpMatrix matrix;
...
void CMemery::CopyPixels(BYTE *DataDes, int strideDes, int leftDes, int topDes, int rightDes, int bottomDes,
                         BYTE *DataSrc, int strideSrc, int leftSrc, int topSrc)
{
...
BOOL CScale::Scale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride, int srcLeft,
                   int srcTop, int srcWidth, int srcHeight, real sharpenCoefficient)
{
    ...
    for (int row = 0; row < top; row++)
    {
        ...
        {
            real srcx = col * rationW + srcLeft;
            ThreeOrderBorder(DataSrc, srcStride, srcLeft, srcTop, srcWidth, srcHeight, srcx, srcy,
                             DataDes + row * desWidth * 4 + col * 4);
        }
    }
...
void CScale::ThreeOrder(const BYTE *DataSrc, const int srcStride, const real fx, const real fy,
                        BYTE *resultColor)
{
    int x0 = (int)(fx);    //floor
    int y0 = (int)(fy);    //floor
...
void CScale::ThreeOrderBorder(const BYTE *DataSrc, const int srcStride, int srcLeft, int srcTop,
                              const int srcWidth, const int srcHeight, const real fx, const real fy, BYTE *resultColor)
{
    int x0 = (int)(fx);    //floor
    int y0 = (int)(fy);    //floor
    real fu = (fx - x0) * 256;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        GetBorderColor(DataSrc, srcStride, row, x0   < srcLeft ? srcLeft : (x0   > srcRight ? srcRight : x0),
                       &pixel[i][0]);
        GetBorderColor(DataSrc, srcStride, row, x0 + 1 < srcLeft ? srcLeft : (x0 + 1 > srcRight ? srcRight : x0 + 1),
                       &pixel[i][4]);
        GetBorderColor(DataSrc, srcStride, row, x0 + 2 < srcLeft ? srcLeft : (x0 + 2 > srcRight ? srcRight : x0 + 2),
                       &pixel[i][8]);
        GetBorderColor(DataSrc, srcStride, row, x0 + 3 < srcLeft ? srcLeft : (x0 + 3 > srcRight ? srcRight : x0 + 3),
                       &pixel[i][12]);
    }
...
BOOL CScale::ImageScale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride,
                        int srcLeft, int srcTop, int srcWidth, int srcHeight, int interpolationMode/* = 0*/)
{
    static CScale s_instance;

    if (interpolationMode == 0)
    {
        return s_instance.Scale(DataDes, desWidth, desHeight, DataSrc, srcStride, srcLeft, srcTop, srcWidth,
                                srcHeight);
    }
    else if (interpolationMode == 1)
    {
        return s_instance.BilinearScale(DataDes, desWidth, desHeight, DataSrc, srcStride, srcLeft, srcTop,
                                        srcWidth, srcHeight);
    }
    else if (interpolationMode == 2)
    {
        return s_instance.Scale(DataDes, desWidth, desHeight, DataSrc, srcStride, srcLeft, srcTop,
                                srcWidth, srcHeight, 0.6f);
    }
...
BOOL CScale::BilinearScale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride,
                           int srcleft, int srctop, int srcWidth, int srcHeight)
{
    //bounder
    real left = (real)desWidth / srcWidth;
...
BOOL CAlphaBlend::AlphaBlend(BYTE *desData, int desStride, int desLeft, int desTop, int desWidth, int desHeight,
                             BYTE *srcData, int srcStride, int srcLeft, int srcTop, BYTE sourceAlpha/* = 255*/)
{
    if (sourceAlpha == 0)
    {
        return TRUE;
...
BOOL CAlphaBlend::AlphaBlend(BYTE *desData, int desStride, int desLeft, int desTop, int desWidth, int desHeight,
                             BYTE *srcData, int srcStride, int srcLeft, int srcTop, int srcWidth, int srcHeight,
                             BYTE sourceAlpha/* = 255*/, int interpolationMode/* = 0*/)
{
    if (srcWidth == desWidth || desHeight == srcHeight)
    {
        ...
        return AlphaBlend(desData, desStride, desLeft, desTop, desWidth, desHeight, srcData, srcStride, srcLeft,
                          srcTop, sourceAlpha);
    }
    ...
    if (!CScale::ImageScale(pDataScale, desWidth, desHeight, srcData, srcStride, srcLeft, srcTop, srcWidth,
                            srcHeight, interpolationMode))
    {
        delete [] pDataScale;
        return FALSE;
    }
    ...
    {
        if (!AlphaBlend(desData, desStride, desLeft, desTop, desWidth, desHeight, pDataScale,
                        desWidth * 4, 0, 0, sourceAlpha))
        {
            delete [] pDataScale;
            return FALSE;
        }
