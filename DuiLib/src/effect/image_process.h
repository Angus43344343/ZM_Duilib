...
public:
    ...
    static BOOL WarpImage(BYTE *pData, RECT &rcSrc, WarpRect &rcDes, RECT &rcBound, BYTE *imgDes,
    ...
    // 外部分配内存的方式
    //remark  rcsrc 是pData的下标范围 rcBound是图片的大小裁剪范围（如rcSrc 0,0,400,300-->rcBound 0,0,401,301)
    static BOOL WarpImage(BYTE *pData, int srcDataWidth, int srcDataHeight, RECT &rcSrc, WarpRect &rcDes,
...
public:
    ...
    // 拷贝数据
    static void CopyPixels(BYTE *DataDes, int strideDes, int leftDes, int topDes, int rightDes, int bottomDes,
...
public:
    ...
    // color  百叶的颜色
    // output Data
    static BOOL BlindsH(BYTE *Data, int width, int height, int iBlindHeight, int iBlindNum, int offsetDistance,
                        DWORD color = 0);
    // 将图片处理成百叶窗特效
    // input Data
    // input width height 图像的长和宽
    ...
    // color  百叶的颜色
    // output Data
    static BOOL BlindsV(BYTE *Data, int width, int height, int iBlindWidth, int iBlindNum, int offsetDistance,
                        DWORD color = 0);
    // 将图片以百叶方式拷贝特效
    // input DataDes
    // input DataSrc
    ...
    // offset 每个百叶效果要处理的行距离自己首行的距离（像素）
    // output DataDes
    static BOOL BlindsH(BYTE *DataDes, BYTE *DataSrc, int width, int height, int iBlindHeight, int iBlindNum,
                        int offsetDistance);
    // 将图片以百叶方式拷贝特效
    // input DataDes
    // input DataSrc
    ...
    // offset 每个百叶效果要处理的列距离自己首列的距离（像素）
    // output DataDes
    static BOOL BlindsV(BYTE *DataDes, BYTE *DataSrc, int width, int height, int iBlindWidth, int iBlindNum,
                        int offsetDistance);
};
...
public:
    ...
    static BOOL ImageScale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride, int srcLeft,
                           int srcTop, int srcWidth, int srcHeight, int interpolationMode = 0);
...
private:
    ...
    // 高质量插值算法（三次卷积插值）
    BOOL Scale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride, int srcLeft, int srcTop,
               int srcWidth, int srcHeight, real sharpenCoefficient);
    // 低质量插值算法（临近点插值）
    BOOL Scale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride, int srcleft, int srctop,
               int srcWidth, int srcHeight);
    // 中质量插值算法（双线性插值）
    BOOL BilinearScale(BYTE *DataDes, int desWidth, int desHeight, BYTE *DataSrc, int srcStride, int srcleft,
                       int srctop, int srcWidth, int srcHeight);
...
private:
    ...
    // diff size
    static BOOL AlphaBlend(BYTE *desData, int desStride, int desLeft, int desTop, int desWidth, int desHeight,
