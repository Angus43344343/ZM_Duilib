...
public:
    int nCompare(const ELT *pcsz, int len = 0) const;
...
template <class ELT> int CBufferRefT<ELT>::nCompare(const ELT *pcsz, int len) const
{
    ...
    for (int i = 0; i < len; ++i)
    {
        if (m_pBuffer[i] != pcsz[i]) { return m_pBuffer[i] - pcsz[i]; }
    }
...
public:
    ...
    void SetMaxLength(int nSize)
    {
        ...
        {
            if (m_nMaxLength < 8)     { m_nMaxLength = 8; }

            if (nSize > m_nMaxLength) { m_nMaxLength *= 2; }
...
template <class ELT> void CBufferT<ELT>::Prepare(int index, int fill)
{
    ...
    if (nNewSize > m_nMaxLength)
    {
        ...
        if (nNewSize > nNewLength) { nNewLength *= 2; }
...
template <class T> int CSortedBufferT <T>::FindAs(const T &rT, int(* compare)(const void *, const void *))
{
    ...
    if (pT != NULL) { return pT - CBufferRefT<T>::m_pBuffer; }
    else            { return -1; }
}
...
template <class CHART> int CBackrefElxT <CHART>::Match(CContext *pContext) const
{
    ...
    if (m_brightleft)
    {
        ...
        //if (npos < slen) { return 0; }
        //if (m_bignorecase) { bsucc = ! refstr.nCompareNoCase(pcsz + (npos - slen)); }
        //else { bsucc = ! refstr.nCompare(pcsz + (npos - slen)); }
        //if (bsucc)
        //{
        //    pContext->m_stack.Push(npos);
    ...
    else
    {
        ...
        //if (npos + slen > tlen) { return 0; }
        //if (m_bignorecase) { bsucc = ! refstr.nCompareNoCase(pcsz + npos); }
        //else { bsucc = ! refstr.nCompare(pcsz + npos); }
        //if (bsucc)
        //{
        //    pContext->m_stack.Push(npos);
        //    pContext->m_nCurrentPos += slen;
...
template <class CHART> int CBoundaryElxT <CHART>::Match(CContext *pContext) const
{
    ...
    case BOUNDARY_FILE_END_N:
        bsucc = (npos >= tlen) || (pcsz[tlen - 1] == RCHART('\n') && (npos == tlen - 1 ||
                                   (pcsz[tlen - 2] == RCHART('\r') && npos == tlen - 2)));
        break;
        ...
        int number)
{
    ...
    while (index >= 0)
    {
        if (pContext->m_capturestack[index] == number) { return 1; }
...
template <class CHART> int CStringElxT <CHART>::Match(CContext *pContext) const
{
    ...
    if (m_brightleft)
    {
        ...
        //else { bsucc = !m_szPattern.nCompare(pcsz + (npos - slen)); }
        //if (bsucc) { pContext->m_nCurrentPos -= slen; }
    }
    else
    {
        // 2019-04-22 zhuyadong 仅用于编辑框正则过滤，比较到字符串结尾。
        ...
        //if (npos + slen > tlen)
        //{ return 0; }
        //if (m_bignorecase) { bsucc = ! m_szPattern.nCompareNoCase(pcsz + npos); }
        //else { bsucc = ! m_szPattern.nCompare(pcsz + npos); }
        //if (bsucc) { pContext->m_nCurrentPos += slen; }
    }
...
template <class CHART> int CStringElxT <CHART>::MatchNext(CContext *pContext) const
{
    ...
    if (m_brightleft) { pContext->m_nCurrentPos += slen; }
    else              { pContext->m_nCurrentPos -= slen; }
...
template <class CHART> int CConditionElxT <CHART>::Match(CContext *pContext) const
{
    ...
    {
        if (m_pelxask == 0) { condition_yes = 1; }
        else                { condition_yes = m_pelxask->Match(pContext); }
    ...
    if (condition_yes) { bsucc = m_pelxyes == 0 ? 1 : m_pelxyes->Match(pContext); }
    else               { bsucc = m_pelxno  == 0 ? 1 : m_pelxno ->Match(pContext); }
...
template <class CHART> int CConditionElxT <CHART>::MatchNext(CContext *pContext) const
{
    ...
    if (condition_yes) { bsucc = m_pelxyes == 0 ? 0 : m_pelxyes->MatchNext(pContext); }
    else               { bsucc = m_pelxno  == 0 ? 0 : m_pelxno ->MatchNext(pContext); }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::Build(const CBufferRefT <CHART> &pattern, int flags)
{
    ...
    for (i = 1; i < m_nGroupCount; ++i)
    {
        ...
        if (pleft->m_nnumber > m_nMaxNumber) { m_nMaxNumber = pleft->m_nnumber; }
    }
    ...
    for (i = 0; i < m_recursivelist.GetSize(); ++i)
    {
        ...
        {
            if (m_recursivelist[i]->m_ndata == 0) { m_recursivelist[i]->m_pelx = m_pTopElx; }
...
template <class CHART> int CBuilderT <CHART>::GetNext2()
{
    ...
    if (m_bQuoted)
    {
        ...
        if (m_quote_fun != 0) { nex2 = CHART_INFO((CHART)(*m_quote_fun)((int)ch), 0, m_nNextPos, delta); }
        else                  { nex2 = CHART_INFO(ch, 0, m_nNextPos, delta); }
            ...
            */
            ...
            case RCHART('x'):
                if (m_pattern[m_nNextPos + 2] != '{')
                {
                    ...
                    if (red > 0) { nex2 = CHART_INFO(RCHART(ch2), 0, m_nNextPos, delta); }
                    else         { nex2 = CHART_INFO(ch1, 0, m_nNextPos, delta); }
    ...
    case RCHART('|'):
    case RCHART('$'):
        if (m_nCharsetDepth > 0) { nex2 = CHART_INFO(ch, 0, m_nNextPos, delta); }
        else                     { nex2 = CHART_INFO(ch, 1, m_nNextPos, delta); }

        break;

    case RCHART('-'):
        if (m_nCharsetDepth > 0) { nex2 = CHART_INFO(ch, 1, m_nNextPos, delta); }
        else                     { nex2 = CHART_INFO(ch, 0, m_nNextPos, delta); }
    ...
    case RCHART('('):
        {
            ...
            if (ch1 == RCHART('?') && ch2 == RCHART('#'))
            {
                ...
                {
                    if (m_pattern[m_nNextPos] == RCHART(')')) { break; }
    ...
    case RCHART('#'):
        if (m_nFlags & EXTENDED)
        {
            ...
            while (m_nNextPos < m_pattern.GetSize())
            {
                if (m_pattern[m_nNextPos] == RCHART('\n') || m_pattern[m_nNextPos] == RCHART('\r')) { break; }
    ...
    case RCHART(':'):
        if (next == CHART_INFO(RCHART('['), 1)) { nex2 = CHART_INFO(ch, 1, m_nNextPos, delta); }
        else                                    { nex2 = CHART_INFO(ch, 0, m_nNextPos, delta); }

        break;

    case RCHART('^'):
        if (m_nCharsetDepth == 0 || next == CHART_INFO(RCHART('['), 1) ||
            (curr == CHART_INFO(RCHART('['), 1) && next == CHART_INFO(RCHART(':'), 1)))
        { nex2 = CHART_INFO(ch, 1, m_nNextPos, delta); }
        else
        { nex2 = CHART_INFO(ch, 0, m_nNextPos, delta); }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::GetStockElx(int nStockId)
{
    ...
    // check
    if (nStockId < 0 || nStockId >= STOCKELX_COUNT) { return GetStockElx(0); }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::BuildRepeat(int &flags)
{
    ...
    {
        // 0 times
        if (nMax == 0) { return GetStockElx(STOCKELX_EMPTY); }

        // fixed times
        if (nMin == nMax)
        {
            if (curr == CHART_INFO(RCHART('?'), 1) || curr == CHART_INFO(RCHART('+'), 1)) { MoveNext(); }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::BuildSimple(int &flags)
{
    ...
    while (curr != CHART_INFO(0, 1))
    {
        if (curr.type == 0)
        {
            if (next == CHART_INFO(RCHART('{'), 1) || next == CHART_INFO(RCHART('?'), 1) ||
                next == CHART_INFO(RCHART('*'), 1) || next == CHART_INFO(RCHART('+'), 1))
            {
                if (fixed.GetSize() == 0)
                {
        ...
        else if (curr.type == 1)
        {
            ...
            // end of simple
            if (vch == RCHART(')') || vch == RCHART('|')) { break; }

            // has fixed already
            if (fixed.GetSize() > 0) { break; }

            // left parentheses
            if (vch == RCHART('(')) { return BuildRecursive(flags); }
    ...
    case RCHART('['):
        {
            ...
            while (curr != CHART_INFO(0, 1) && curr != CHART_INFO(RCHART(']'), 1))
            {
                ...
                if (curr.type == 1 && (
                        ch == RCHART('.') || ch == RCHART('w') || ch == RCHART('W') || ch == RCHART('s') || ch == RCHART('S') ||
                        ch == RCHART('d') || ch == RCHART('D') ||
        ...
        default:
            ...
            else
            {
                       ...
                       curr != CHART_INFO(RCHART('('), 1))
                {
                    ...
                    if (bNegative) { newflags &= ~tochange; }
                    else           { newflags |=  tochange; }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::BuildBoundary(int &flags)
{
    ...
    case RCHART('G'):
        if (flags & GLOBAL) { return Keep(new CGlobalElx()); }
        else                { return GetStockElx(STOCKELX_EMPTY); }
...
template <class CHART> ElxInterface *CBuilderT <CHART>::BuildBackref(int &flags)
{
    ...
    if (curr.ch == RCHART('<') || curr.ch == RCHART('\''))
    {
        ...
        CBackrefElxT <CHART> *pbackref = (CBackrefElxT <CHART> *)Keep(new CBackrefElxT <CHART> (-1,
                                         flags & RIGHTTOLEFT, flags & IGNORECASE));
...
template <class CHART> int CBuilderT <CHART>::ReadDec(char *&str, unsigned int &dec)
{
    ...
    {
        if (str[i] >= '0' && str[i] <= '9') { dec = dec * 10 + (str[i] - '0'); }
        else                                { break; }
    }
...
public:
    ...
    CHART *Replace(const CHART *tstring, const CHART *replaceto, int start = -1, int ntimes = -1,
                   MatchResult *result = 0, CContext *pContext = 0) const;
    CHART *Replace(const CHART *tstring, int string_length, const CHART *replaceto, int to_length,
                   int &result_length, int start = -1, int ntimes = -1, MatchResult *result = 0, CContext *pContext = 0) const;
    ...
    while (((*result) = rtoreg.Match(pContext)).IsMatched())
    {
        ...
        default:
            ...
            for (delta = 1; delta <= 3; ++delta)
            {
                ...
                if (ch < RCHART('0') || ch > RCHART('9')) { break; }
...
template <int x> int CAssertElxT <x>::Match(CContext *pContext) const
{
    ...
    if (bsucc) { pContext->m_stack.Push(ncsize); }
    else       { pContext->m_capturestack.Restore(ncsize); }
...
template <int x> int CGreedyElxT <x>::MatchVart(CContext *pContext) const
{
    ...
    while (n < m_nvart && CRepeatElx::MatchForward(pContext)) { ++n; }
